...
CREATE TABLE contributions_made ( contribution_id INT PRIMARY KEY AUTO_INCREMENT, event_contri_id INT NOT NULL, student_id INT NOT NULL, paid_amount DECIMAL(10,2) NOT NULL DEFAULT 0, payment_type VARCHAR(20), online_payment_proof VARCHAR(50), paid_date DATE NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (event_contri_id) REFERENCES event_contributions(event_contri_id) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY (student_id) REFERENCES students(student_id) ON DELETE CASCADE ON UPDATE CASCADE ) AUTO_INCREMENT = 1001;
